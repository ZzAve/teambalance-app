...
data class TeamMember(
    ...
    // The member's *permission* within the team — "USER" or "ADMIN" (the `team_members.role` column,
    // CHECK IN ('USER','ADMIN')). This is access level, NOT the volleyball position (see [position]).
    // The two MUST NOT be conflated (PRD #8): permission governs what a member may do; position is
    // where they play. Kept a raw String to mirror the wire contract's `Member.role`.
