...
function EventListPage() {
  ...
  useEffect(() => {
    if (eventTypes && activeTypeIds.size === 0) {
      setActiveTypeIds(new Set(eventTypes.map(t => t.id)))
    }
  }, [eventTypes])
  ...
  return (
    <div>
      ...
      {eventTypes && eventTypes.length > 0 && (
        <div className="-mx-4 mt-3 flex gap-2 overflow-x-auto px-4 pb-0.5 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          {eventTypes.map((type) => {
