import{n as e}from"./rolldown-runtime-DkW27tQK.js";import{r as t,s as n}from"./event-fixtures-CuRrQuRB.js";import{t as r}from"./jsx-runtime-DeHZSEgm.js";import{n as i,t as a}from"./RosterPips-2FiGTd8c.js";var o,s,c,l,u,d,f,p,m,h,g,_,v,y,b,x,S,C,w;function T(){return(T=e((()=>{t(),i(),o=r(),{expect:s}=__STORYBOOK_MODULE_TEST__,c=(e,t,n,r=`PLAYING`)=>({id:`pos-${e.toLowerCase()}`,label:e,required:t,attending:n,kind:r}),l={title:`entities/event/RosterPips`,component:a,decorators:[e=>(0,o.jsx)(`div`,{className:`max-w-xs rounded-xl border border-border bg-card p-3.5`,children:(0,o.jsx)(e,{})})]},u={args:{roster:n({state:`LINEUP_SET`,openSlots:0,totalAttending:5,positions:[c(`Setter`,2,2),c(`Libero`,1,1),c(`Middle`,2,2)]})},play:async({canvas:e})=>{await s(e.getByText(`3 of 3 covered`)).toBeInTheDocument(),await s(e.getAllByText(`2/2`)).toHaveLength(2),await s(e.getByText(`1/1`)).toBeInTheDocument(),await s(e.queryByText(/the one to chase/)).not.toBeInTheDocument()}},d={args:{roster:n({state:`SPOTS_OPEN`,totalAttending:3,positions:[c(`Setter`,2,1),c(`Libero`,1,1),c(`Middle`,2,1)]})},play:async({canvas:e})=>{await s(e.getByText(`1 of 3 covered`)).toBeInTheDocument(),await s(e.queryByText(/the one to chase/)).not.toBeInTheDocument()}},f={args:{roster:n({state:`CRITICAL`,totalAttending:3,positions:[c(`Setter`,2,2),c(`Libero`,1,0),c(`Middle`,2,1)]})},play:async({canvas:e})=>{await s(e.getByText(/still has no one/)).toBeInTheDocument(),await s(e.getByText(`Libero`,{selector:`b`})).toBeInTheDocument(),await s(e.getByText(`0/1`)).toBeInTheDocument()}},p={args:{roster:n({state:`CRITICAL`,totalAttending:2,positions:[c(`Setter`,2,2),c(`Libero`,1,0),c(`Middle`,2,0)]})},play:async({canvas:e})=>{await s(e.getByText(`Libero and Middle`,{selector:`b`})).toBeInTheDocument(),await s(e.getByText(/still have no one/)).toBeInTheDocument(),await s(e.queryByText(/the one to chase/)).not.toBeInTheDocument()}},m={args:{roster:n({state:`CRITICAL`,totalAttending:0,positions:[c(`Diagonaal`,2,0),c(`Libero`,1,0),c(`Midden`,3,0),c(`Passer/Loper`,2,0),c(`Spelverdeler`,2,0),c(`Trainer/Coach`,1,0)]})},play:async({canvas:e})=>{await s(e.getByText(`6 positions`,{selector:`b`})).toBeInTheDocument(),await s(e.getByText(/still have no one/)).toBeInTheDocument(),await s(e.queryByText(/the one to chase/)).not.toBeInTheDocument(),await s(e.queryByText(`Diagonaal`,{selector:`b`})).not.toBeInTheDocument(),await s(e.getByText(`0 of 6 covered`)).toBeInTheDocument()}},h={args:{roster:n({state:`HEADCOUNT_SHORT`,openSlots:4,totalTarget:10,totalAttending:6,positions:[c(`Setter`,void 0,4),c(`Middle`,void 0,2)]})},play:async({canvas:e})=>{await s(e.queryByText(/covered/)).not.toBeInTheDocument(),await s(e.getByText(`4`)).toBeInTheDocument(),await s(e.getByText(`6/10 going`)).toBeInTheDocument()}},g={args:{roster:n({state:`CRITICAL`,openSlots:1,totalAttending:5,positions:[c(`Setter`,2,5),c(`Libero`,1,0)]})},play:async({canvas:e})=>{await s(e.getByText(`+3`)).toBeInTheDocument(),await s(e.getByText(`5/2`)).toBeInTheDocument(),await s(e.getByText(/still has no one/)).toBeInTheDocument()}},_={args:{roster:n({state:`HEADCOUNT_SHORT`,openSlots:4,totalTarget:10,totalAttending:6,unassignedAttending:6,positions:[]})},play:async({canvas:e})=>{await s(e.queryByText(`Nobody has answered yet.`)).not.toBeInTheDocument(),await s(e.getByText(`6 going haven't set a position`)).toBeInTheDocument(),await s(e.getByText(`6/10 going`)).toBeInTheDocument()}},v={args:{roster:n({state:`TALLY_ONLY`,openSlots:0,totalTarget:void 0,totalAttending:0,positions:[]})},play:async({canvas:e})=>{await s(e.getByText(`Nobody has answered yet.`)).toBeInTheDocument()}},y={args:{roster:n({state:`SPOTS_OPEN`,openSlots:5,totalAttending:7,positions:[c(`Squad`,12,7)]})},play:async({canvas:e})=>{await s(e.getByText(`7/12`)).toBeInTheDocument()}},b={args:{roster:n({state:`SPOTS_OPEN`,totalAttending:6,unassignedAttending:3,positions:[c(`Setter`,2,1),c(`Libero`,1,1),c(`Middle`,2,1)]})},play:async({canvas:e})=>{await s(e.getByText(`3 going haven't set a position`)).toBeInTheDocument(),await s(e.getByText(`1 of 3 covered`)).toBeInTheDocument()}},x={args:{roster:n({state:`SPOTS_OPEN`,openSlots:1,totalTarget:12,totalAttending:7,positions:[c(`Setter`,2,1),c(`Libero`,1,1)]})},play:async({canvas:e})=>{await s(e.getByText(`1 of 2 covered`)).toBeInTheDocument(),await s(e.getByText(`7/12 going`)).toBeInTheDocument()}},S={args:{roster:n({state:`HEADCOUNT_SHORT`,openSlots:1,totalTarget:12,totalAttending:12,positions:[c(`Setter`,void 0,11),c(`Trainer`,void 0,1,`STAFF`)]})},play:async({canvas:e})=>{await s(e.getByText(`11/12 going`)).toBeInTheDocument(),await s(e.getByText(`1 staff also going, not counted toward the target`)).toBeInTheDocument(),await s(e.getByText(`Trainer`)).toBeInTheDocument()}},C={args:{roster:n({state:`LINEUP_SET`,totalTarget:6,totalAttending:4,positions:[c(`Setter`,2,2),c(`Libero`,1,1),c(`Trainer`,void 0,1,`STAFF`)]})},play:async({canvas:e})=>{await s(e.getByText(`2 of 2 covered`)).toBeInTheDocument(),await s(e.getByText(`3/6 going`)).toBeInTheDocument(),await s(e.getByText(`1 staff also going, not counted toward the target`)).toBeInTheDocument()}},u.parameters={...u.parameters,docs:{...u.parameters?.docs,source:{originalSource:`{
  args: {
    roster: makeRoster({
      state: 'LINEUP_SET',
      openSlots: 0,
...
}`,...C.parameters?.docs?.source}}},w=[`LineupSet`,`SpotsOpen`,`Critical`,`TwoEmptyPositions`,`NothingAnsweredYet`,`HeadcountOnly`,`WithSurplus`,`AllAttendeesUnassigned`,`NobodyAttending`,`WithManySlots`,`WithUnassigned`,`BothAxes`,`WithStaffAttending`,`WithStaffAndPositionTargets`]})))()}T();export{_ as AllAttendeesUnassigned,x as BothAxes,f as Critical,h as HeadcountOnly,u as LineupSet,v as NobodyAttending,m as NothingAnsweredYet,d as SpotsOpen,p as TwoEmptyPositions,y as WithManySlots,C as WithStaffAndPositionTargets,S as WithStaffAttending,g as WithSurplus,b as WithUnassigned,w as __namedExportsOrder,l as default};