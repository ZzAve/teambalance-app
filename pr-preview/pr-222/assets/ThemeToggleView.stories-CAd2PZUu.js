...
}`,...k.parameters?.docs?.source},description:{story:"The same control under the dark token layer. `.dark` on a wrapper is enough: the `dark:` variant\nis `&:is(.dark *)` and the token overrides are custom properties, which inherit — so this renders\nfrom exactly the variables a dark-mode app does, and Chromatic diffs them.",...k.parameters?.docs?.description}}},A=[`SystemSelected`,`LightSelected`,`DarkSelected`,`ChoosingDark`,`ReturningToSystem`,`Dark`]})))()}j();export{D as ChoosingDark,k as Dark,E as DarkSelected,T as LightSelected,O as ReturningToSystem,w as SystemSelected,A as __namedExportsOrder,C as default};